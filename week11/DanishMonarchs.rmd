...
- read_csv is the tidyverse function
...
- read.csv, read_csv and read_csv2 returns 57, read.csv2 returns 59

4. Show the dataset so that we can see how R interprets each column

- This is done with the glimpse tidyverse function
...
# 25 kings enjoyed a longer than average reign
# not sure if this is correct, as the second tibble does look a bit weird
```
...
kings_totaldaysreigned <- kings %>% 
  filter(!is.na(duration)) %>% 
  select(Name, duration) %>% 
  mutate(total_days_reigned = duration *  365)

  kings_totaldaysreigned %>% 
    arrange(desc(total_days_reigned)) %>% 
    slice_head(n = 3)
# could not figure out a smart way to do transition year
```
...
kings_midyear %>% 
  ...
  geom_smooth() +
  geom_line() + 
  labs(title = "Danish Monarchs reign over time", x = "Centuries", y = "Years reigned") +
  theme_bw(base_size = 14)
# I added geom_line as well, as I thought it also added a better visualisation
```
