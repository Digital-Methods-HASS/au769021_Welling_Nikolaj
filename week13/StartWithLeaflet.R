...
# It is a shortening of TRUE or FALSE. If layerControlOptions(collapsed) is set to true, the layers menu will be collapse.
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView(9.779505, 55.531049, zoom = 13) %>% 
  addTiles()
l_dk
...
# In short: Just pasted two of the code blocks together
...
# adding clusterOptions = markerClusterOptions() in the addMarkers() line
...
# Your brief answer:
# Clustering is better for getting a clearer image of where these markers mostly reside,
# but dont give a very clear image of where the markers specifically are. You can call the clusters distant reading,
# and then without markers, close reading.
...
# It already did. After I had used some of the earlier code
# It is specifically the "popup = paste(places$Description, "<br>", places$Type)" line.
